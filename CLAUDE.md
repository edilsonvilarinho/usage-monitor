...
- **Última leitura das fontes locais** (`LOCAL_SESSION_CACHE_SOURCES` em `DashboardViewModel`): Gemini, Cursor e Antigravity mantêm a última leitura depois de uma falha, como o Codex, e ela ganha `ApiUsageNotice.SOURCE_UNSTABLE`. Sem a marca o card mostrava números congelados como se fossem da coleta corrente. O texto do aviso depende da fonte (`noticeText`): a frase do Codex fala do contrato dele, e nas outras o aviso é "última leitura válida" — valor novo no enum seria mais um `when` a quebrar pela mesma informação.
- **Antigravity CLI** (`LocalAntigravityUsageDataSource` + `AntigravityUsageMapper` + `AntigravityQuotaLabels`; issue #267, plano [`integracoes-267-ajustes-execucao.md`](docs/planos/integracoes-267-ajustes-execucao.md)): cotas lidas por `agy --sandbox --print-timeout 30s --output-format json --print /usage`, **sem PTY**. A primeira versão dirigia a TUI interativa por pty4j e nunca abriu o painel: o pty4j usa winpty por default no Windows, `/usage⏎` ia às cegas depois de 900 ms, e o parser procurava cabeçalhos adivinhados.
...
- **`/metrics` é a única rota em que o servidor precifica** (OpenMetrics, servidor 0.12.0+, issue #167, plano [`metricas-prometheus-167-execucao.md`](docs/planos/metricas-prometheus-167-execucao.md)). `/v1/report/*` continua devolvendo linha crua e `/v1/pricing` continua publicando a tabela; aqui não há essa opção, porque o Prometheus ingere números e não aplica tabela de preço — a alternativa seria o operador manter uma cópia da tabela à mão, que é exatamente o que a #105 provou divergir. A aritmética vive em `server/src/domain/usageCost.ts`, espelhada de `ModelPricing.kt`.
  - **BigInt, não `number`.** O `number` do JS é double e perde exatidão acima de 2^53; 10^11 tokens de cache read × 5×10^6 micros/milhão dá 5×10^17, e um time real chega lá em sete dias. Sem BigInt o custo sairia errado **sem erro nenhum** — há teste afirmando exatamente esse caso.
  - **Gauges de janela deslizante (24h e 7d), nunca counters.** O counter é o idioma do Prometheus, mas a poda de retenção faz o total **cair**, e queda parcial é lida como reset de contador: a taxa sairia inflacionada num dia qualquer, em silêncio. O preço aceito — `rate()` não se aplica — está escrito no README.
  - **`session_id`, `cwd` e branch não são rótulos.** Toda linha vira uma série e o id de sessão é ilimitado: a cardinalidade cresceria para sempre, que é a forma clássica de derrubar um servidor de métricas. Para o corte por sessão existe `/v1/report/usage`.
  - **Nome de pessoa fica em `_info`, nunca em série de valor.** `alias` é texto digitado e mutável, e como rótulo de valor renomear a máquina cria série nova e quebra o gráfico no meio. As séries de valor levam só `account` e `member` (= `device_id`), que são estáveis; a junção é no PromQL.
  - **Modelo sem tarifa não vira custo zero**: sai em `usage_monitor_unpriced_turns_window`, mesma regra de `CliUsageBucket.isCostComplete`. Sem ela um painel mostraria queda de gasto onde houve modelo novo.
  - **`Authorization: Bearer` foi acrescentado a `requireGlobalRead`**, ao lado de `x-report-key`/`x-admin-token`: o `scrape_config` do Prometheus manda `authorization` nativamente, e header de nome próprio não é garantido em todo agente. É o mesmo segredo por outro transporte, e por isso vale para toda a família de leitura global. `requireAdminToken` **não foi tocado** e continua sendo o portão único dos `DELETE`.
  - **Rótulo de texto livre é escapado.** Uma aspa num apelido invalida o documento **inteiro**, não a linha — o scrape falha por completo.
  - **Acima de `TEAM_METRICS_MAX_SERIES` o rótulo `model` é agregado fora** e `usage_monitor_metrics_model_label_dropped` vai a 1: degradar com sinal, e não truncar em silêncio nem responder 500, que quebraria o scrape por causa de uma dimensão. Mesma regra de "janela ausente degrada, resposta vazia falha".
  - **`SERVER_VERSION` é um segundo dono do número da versão**, porque o `Dockerfile.dokploy` copia só `tsconfig.json` e `src` e ler o `package.json` do `dist/` exigiria sair dele. O preço é `test/unit/version.test.ts`, que compara os dois.
...
- **Sessão ativa e atenção são movimento contínuo, atrás da política**: o arco fino que gira por
  dentro do anel (turno CLI nos últimos 5 min) e o pulso do
