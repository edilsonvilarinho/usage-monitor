...
- **Métricas por ferramenta** (`cli_turn_tools` + `CliToolUsage`): `ClaudeTranscriptMessageDto.content` é lido como `JsonElement` **cru** porque o campo é polimórfico — array de blocos nas linhas do assistente, string em outras; um tipo fixo faria o parse da linha inteira falhar num dos dois casos. Só o **nome** da ferramenta é extraído, nunca o `input` nem o texto. A tabela nasceu vazia para tudo que já estava indexado, e `syncIndex` só reprocessa arquivo alterado: por isso existe `PRAGMA user_version` e `INDEX_SCHEMA_VERSION` — subir o número zera `last_offset` e força a releitura completa. O reset não apaga sessões nem turnos, que voltam pelos mesmos `INSERT OR IGNORE`/`OR REPLACE` e não duplicam. **Ferramenta não entra em custo**: um turno que chama `Read` e `Bash` gastou tokens uma vez só, e ratear o custo entre as duas contaria o mesmo gasto duas vezes — a tela diz isso.
