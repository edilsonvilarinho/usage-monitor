...
export function AppConfirmationDialog({ title, message, confirmLabel, cancelLabel, onConfirm, onDismiss, style }) {
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--r10)', padding: 'var(--s4)', display: 'flex', flexDirection: 'column', gap: 'var(--s3)', maxWidth: 520, boxShadow: 'var(--e8)', ...style }}>
      <span style={{ fontFamily: 'var(--mono)', fontSize: 'var(--t13)', fontWeight: 600 }}>{title}</span>
