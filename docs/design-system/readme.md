...
| --- | --- | --- |
| `AppGroupBand` | Component, `components/data/` | The quiet account sub-band. Team usage and presence both drew it by hand — it is the third step of the surface ladder the prototype draws but does not name. **Not** the panel header: that one speaks loud (`--t12` on `--fg`), this one speaks low (`--t10` on `--muted`), and swapping them inverts the hierarchy. Not the nested-children pattern either (`guidelines/pattern-nested-guide.html`) — the two answer different questions about aninhamento; see that guideline's note, corrected 2026-09-04 (issue #223) from `--bg` to `--surface` to match `Modifier.appNestedGroupItem`, the real implementation. |
