...
| A10 | Correções vistas com o app em execução | `gradlew.bat allTests` + `gradlew.bat run` com `F1` | `BUILD SUCCESSFUL`, **1842 testes, 0 falhas**. Três defeitos que só o uso mostrou. (1) **A faixa da demo virou teto, não altura fixa**: nesta máquina a área útil é 1280×752 com a escala em 115%, e os 420dp fixos deixavam a seção "Como ativar" abaixo da dobra — ela agora cede até 55% da área rolável, e quem encolhe é a demo, que o `Fit` mantém inteira. (2) **A demo de modos de janela desenhava o HUD por cima dos cards** e as duas exibições se misturaram: a linha do HUD lia como conteúdo de um card. Virou três estados em sequência — janela normal, somente cards, barra HUD —, com o vazio em volta das duas últimas sendo o próprio assunto. (3) **A grade de cards fora de um contêiner rolável é ancorada pelo centro** e o quadro começava no meio de um card; o pano de fundo passou a ser `Column + verticalScroll`, que é a estrutura do dashboard real. E `HelpMediaResourcesTest` reprovou a demo de presença: os 11 quadros saíram idênticos — imagem parada vendida como demo —, corrigida com o filtro "Somente conectados" |
| A11 | README (pt/en), CHANGELOG e o bloco do CLAUDE.md | inspeção do diff | linha em "Recursos"/"Features", nota do `generateHelpMedia` junto da das capturas, entrada em `[Unreleased] / Added` e o parágrafo "Ajuda dentro do app" com as decisões que sobreviveram à execução |
