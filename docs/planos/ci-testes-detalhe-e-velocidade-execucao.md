...
| A11 | 2026-08-25 | `test: make the history view model test wait for real work` | Espera não determinística em teste preexistente | concluída | **Segunda atividade não prevista, e a mais alinhada com a pergunta da issue.** O run `32855876748` reprovou `HistoryViewModelTest > emits Empty state when enabledApis is empty` com `Expected … HistoryUiState$Empty, actual … HistoryUiState$Loading`. Não é intermitência: `delay(20)` dentro de `runTest` avança tempo **virtual** e o `HistoryViewModel` roda em `Dispatchers.Default`, tempo real — o laço de 200 tentativas girava em tempo zero e devolvia o primeiro estado que encontrasse. O `delay(100)` da linha 106, escrito para provar que *nada* acontecia, era pior: passava sem esperar coisa nenhuma. O arquivo irmão já tinha a solução — `pauseForBackgroundWork` (`yield()` + `Thread.sleep(20)`) em `DashboardViewModelTestSupport`. **Prova de que a espera virou real:** a classe passou de `time=0.033s` (XML do CI) para `time=0.85s`, medida três vezes com `cleanDesktopTest desktopTest --tests "…HistoryViewModelTest" --no-build-cache`, 9 testes e 0 falhas nas três. Suíte completa depois do fix: `1293 | 0 | 0 | 0 | 122` |
