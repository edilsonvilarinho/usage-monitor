...
| A02 | 2026-08-25 | `feat(pricing): publish the pricing table from the server` | Tabela TS + guarda de paridade no CI | concluída | `server/src/domain/modelPricing.ts` com `PRICING_VERSION`, `MODEL_PRICING` (12 entradas, mesma ordem do Kotlin), `CACHE_MULTIPLIERS` em razões inteiras, `SYNTHETIC_MODEL_ID` e `MODEL_MATCH_RULE` — as duas últimas já aqui porque são o que a rota da A03 publica, e criá-las lá abriria um segundo dono do mesmo texto. `tools/ci/check-pricing-parity.mjs` compara **posição a posição** e reprova reordenação: as duas listas declaram a mesma tabela, e reordenação sem justificativa é o primeiro sintoma de edição feita num lado só. Razão equivalente também reprova (125/100 contra 5/4) — lado "simplificado" é edição que o outro não recebeu. **Prova de que a guarda morde, medida nas duas direções:** preço divergente no TS (1 divergência), entrada `<synthetic>` removida (3 divergências, incluindo a checagem própria dela), multiplicador reescrito como 125/100 (1), e `SONNET_5` alterada no **Kotlin** (1). Restaurado: `12 modelo(s) e 3 multiplicador(es) ... identicos`. Step em `ci.yml` **e** em `ci-server.yml`, antes do setup de cada um — os filtros de path são disjuntos e um step só deixaria metade das edições passar batida. YAML dos dois validado com `yaml.safe_load`; ordem dos steps conferida. `npm run typecheck`: sem erro. `npm test`: **179 testes / 15 arquivos / 0 falhas** (174 antes) |
| A03 | 2026-08-25 | `feat(report): read globally with a report-only token` | `/api/v1/pricing` + `TEAM_REPORT_TOKEN` | concluída | `reportToken` em `config.ts`, **fora** da checagem de "pelo menos um segredo" — servidor que só publica relatório e não aceita cliente nenhum não tem o que relatar. `requireGlobalRead` novo em `access.ts` aceita `x-admin-token` **ou** `x-report-key` sem escopo de conta; `requireAdminToken` **não foi tocado**, e é por isso que o token de relatório não alcança `DELETE` nenhum. Em `authorize()` ele entra no mesmo ponto do token de admin, dentro do `if (!allowClaim)`, o que já o recusa em ingest e presença. **Desvio do plano:** `/admin/v1/overview` passou a usar um helper `readOnly` no `admin.ts` — sem isso o token de relatório não leria a visão agregada, que a verificação da issue exige; a listagem de chaves continua só no admin porque devolve material de credencial. `report.ts` é montado **incondicionalmente**: sem a variável a rota responde 401, e rota ausente faria "credencial errada" e "variável não definida" chegarem como o mesmo 404. `TEAM_REPORT_TOKEN` documentado em `.env.example`. `npm run typecheck` sem erro; `npm test`: **195 testes / 16 arquivos / 0 falhas** (179 antes), com 12 casos novos em `test/api/report.test.ts` — incluindo os quatro 401 de escrita (`/v1/ingest`, `/v1/presence`, `DELETE /v1/member`, `DELETE /admin/v1/accounts/:accountKey`) e a prova de que nada foi apagado depois deles |
| A04 | 2026-08-25 | `feat(report): close the read window on the right` | Filtro `until` nas três leituras | concluída | `until` opcional em `teamQuerySchema`, `overviewQuerySchema` e `trendQuerySchema`, com refino `until > since` → 400 e `path: ['until']` explícito — sem o `path` a issue nasce sem caminho e a mensagem das rotas sai como "parametros ausentes". As cinco consultas ganharam `AND (@until IS NULL OR t.ts < @until)`; nas duas de atividade ele entra **dentro da subconsulta, antes do `LAG`**. `readTeam`, `readTrend` e `readOverview` mudaram de assinatura — só `team.ts` e `admin.ts` as chamam. `/v1/team/trend` aceita `since`/`until` absolutos com `days` de fallback, e `since` explícito vence. **Correção sobre o texto do plano:** o intervalo que cruza a fronteira não pertence à janela seguinte — ele não é contado em **nenhuma** das duas, e há teste medindo isso (3 min na janela inteira, 1 min em cada metade). `npm run typecheck` sem erro; `npm test`: **205 testes / 17 arquivos / 0 falhas** (195 antes), com 10 casos novos em `test/api/window.test.ts` — inclusive a soma de janelas adjacentes batendo com a janela inteira e as duas requisições que `RemoteTeamDataSource` monta hoje |
| A05 | 2026-08-25 | `feat(report): page the flat report routes by group key` | Rotas `/api/v1/report/*` paginadas | concluída | Três rotas sob `requireGlobalRead`: `/report/usage` e `/report/activity` com cursor, `/report/members` sem — o tamanho dela é o do time, não o do histórico. A ordem é a **chave de agrupamento** (conta, máquina, sessão, modelo) e o predicado do cursor cai no `WHERE`, antes de agrupar; `IFNULL(model, '')` porque NULL não compara em row-value e a página pararia na primeira linha sem modelo. **A consulta pede `limit + 1`**: é a presença do extra, e não a página estar cheia, que prova haver próxima — sem isso a última página exata devolveria um cursor que abriria página vazia, e há teste medindo. No eixo de atividade o cursor filtra `(conta, máquina, sessão)` **inteiras**: cortar turnos dentro de uma sessão mudaria o `LAG` e o tempo dela passaria a depender da página em que caiu — o teste compara a soma paginada de um em um com a leitura inteira. Cursor é base64url opaco; ilegível responde **400**, não 500. `npm run typecheck` sem erro; `npm test`: **216 testes / 17 arquivos / 0 falhas** (205 antes), com 11 casos novos |
| A06 | 2026-08-25 | `docs(report): document the external consumption api` | README, CLAUDE.md, versão 0.10.0 | concluída | `server/package.json` 0.9.0 → 0.10.0. `server/README.md`: `TEAM_REPORT_TOKEN` na tabela de configuração e a nota de que ele **não** satisfaz a exigência de segredo do boot; `x-report-key` na tabela de credenciais, com o que ele lê e o que recusa; `until` nas tabelas de `/v1/team`, `/v1/team/trend` e `/admin/v1/overview`, mais o parágrafo do semiaberto e do intervalo de fronteira; seções novas de `/api/v1/pricing` e `/api/v1/report/*` com os contratos que não são deriváveis da resposta; bullet no **Modelo de segurança** dizendo que o token lê tudo, não escreve nada e não tem rotação nem rate limit; e a nota de que a **0.10.0 não acrescenta tabela nem coluna**. `CLAUDE.md` ganhou quatro parágrafos na seção de integração com time. Protótipo visual **não** atualizado, conforme decisão travada: nenhuma superfície visível muda |
...
| 2026-08-25 | O prefixo `<synthetic>` tem **dois donos** do lado TS: o literal dentro de `MODEL_PRICING`, exigido pelo formato que o parser de paridade lê, e a constante `SYNTHETIC_MODEL_ID` que a rota publica | `server/test/unit/modelPricing.test.ts` afirma que a constante está na lista. O parser cobre a paridade entre os dois arquivos; esta é a invariante que só existe do lado TS |
| 2026-08-25 | Os dois workflows estão em CRLF | Irrelevante para o CI, mas edição por script precisa preservar o fim de linha do arquivo — trocar tudo por LF produziria um diff de arquivo inteiro |
| 2026-08-25 | O plano dizia que `requireAdminToken` não seria tocado, e a verificação exigia que `x-report-key` lesse `/admin/v1/overview` — as duas coisas não cabem juntas, porque o roteiro admin protege tudo com aquele helper | Resolvido sem mexer em `requireAdminToken`: o `admin.ts` ganhou um segundo helper, `readOnly`, aplicado **só** à visão agregada. Todos os `DELETE` e a listagem de chaves continuam no `protect` |
| 2026-08-25 | Com `adminToken` nulo o roteiro admin não é montado, então `/admin/v1/overview` não existe num deploy que só define `TEAM_REPORT_TOKEN` | Aceito: as rotas `/api/v1/report/*` da A05 cobrem o mesmo dado sem depender da administração estar ligada |
| 2026-08-25 | O plano dizia que o intervalo entre turnos que cruza a fronteira "pertence à janela seguinte". Medido: ele não entra em nenhuma das duas — na janela da esquerda falta o turno de destino, na da direita o `LAG` do primeiro turno é nulo | Comentário do repositório corrigido para descrever o que acontece. Contá-lo nas duas duplicaria o tempo, e atribuí-lo a uma exigiria ler turno fora dela; é o que `since` sempre fez na borda esquerda |
| 2026-08-25 | `server/src/repositories/teamRepository.ts` tem fim de linha **misto** (LF e CRLF na mesma região), e o script de edição escreveu um CR literal dentro do próprio registro | Edição por script casa CR opcional antes de cada LF em vez de converter o texto inteiro; conversão em bloco produziria diff do arquivo todo |
| 2026-08-25 | `/report/members` precisa listar conta que já tem chave emitida e ainda não recebeu ingest nenhum | `readReportMembers` semeia a lista pelos rótulos (`team_key_accounts`) antes dos integrantes: sem isso a conta recém-vinculada sumiria do "quem é quem" até o primeiro envio |
| 2026-08-25 | Sessão sem intervalo medido não aparece em `/report/activity` | Mantido: é a mesma promessa de `/v1/team` — uma linha com zero afirmaria "medido e sem intervalo", e ali o correto é "não medido" |
