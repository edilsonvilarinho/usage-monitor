...
| A7 | Chave estável no gesto de arrasto (higiene, não correção) | `gradlew.bat desktopTest --tests "com.usagemonitor.ui.DesktopWindowFrameTest"` | `BUILD SUCCESSFUL`, 16 testes. **Medição:** com as lambdas como chave o arrasto também sobrevive (contador 3 nos dois casos) — a hipótese de que a troca de chave matava o gesto é **falsa** |
| A8 | Painel permanente: uma linha por fonte, com estado, nome, percentual e reset | `gradlew.bat allTests` | `BUILD SUCCESSFUL`, 1804 testes (26 em `HudWindowGeometryTest`, 17 em `DesktopWindowFrameTest`) |
| A9 | Rodapé de consumo: sessões ativas, custo e tokens da janela de 5h | `gradlew.bat allTests` | `BUILD SUCCESSFUL`, 1822 testes (7 do caso de uso, 4 do view model, 7 do formato) |
| A10 | Protótipo, design system e CLAUDE.md para o painel e o rodapé | inspeção do diff + balanceamento de tags do bloco HUD (39/39 `div`, 31/31 `span`) | quatro estados novos no protótipo e no kit, contrato de `AppHudBar` reescrito, bloco do CLAUDE.md atualizado, quatro linhas novas na §15 |
| A11 | Rodapé cortado: a geometria não contava o padding próprio dele | `gradlew.bat desktopTest --tests "com.usagemonitor.ui.HudBarHeightTest"` | `BUILD SUCCESSFUL`, 5 testes. **Com a correção revertida, 2 deles falham** — é a costura que faltava entre a conta pura e o que o Compose dispõe |
