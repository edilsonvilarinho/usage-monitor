...
| A04 | Cursor: formatos enterprise/team, saturação acima de 100, sentinela sem fim de ciclo, total misto fora, falhas tipadas como configuração, rede sem embrulho, token fora do `toString`, SQLite com `busy_timeout` e `immutable=1` | `gradlew.bat desktopTest --tests "com.usagemonitor.data.*Cursor*" --tests "com.usagemonitor.presentation.ui.DashboardScreenWarningsTest" --tests "com.usagemonitor.presentation.UiStateTest" --tests "com.usagemonitor.ui.ComponentTest"` | BUILD SUCCESSFUL. Cursor não está instalado nesta máquina: os formatos ficam cobertos só por fixtures |
| A05 | Cache preservado de Gemini, Cursor e Antigravity marcado com `SOURCE_UNSTABLE`, com texto próprio fora do Codex; conjunto nomeado no lugar do literal; ramo duplicado do `ApiKeyProbe` | `gradlew.bat desktopTest --tests "com.usagemonitor.presentation.DashboardViewModelTest" --tests "com.usagemonitor.ui.ComponentTest"` | BUILD SUCCESSFUL |
| A06 | Design system: justificativa dos acentos restaurada e decisão de `--gemini`/`--cursor` registrada (294° e 90°, 20°+ dos vizinhos); kit, protótipo obrigatório (seção 4e, legenda, APIs, checklist) e cópia em `uploads/` alinhados; captura `img/new-integrations.png` regenerada | `gradlew.bat generateScreenshots` + `gradlew.bat allTests` | BUILD SUCCESSFUL em 4m05s, 2077 testes, 0 falhas, 0 ignorados |
