...
| A03 | 2026-08-29 | `docs: split the technical reference out of the readme` | `docs/integrations.md`, `docs/architecture.md`, `docs/build-and-release.md` | concluída | As ~200 linhas de detalhe que estavam misturadas à documentação de usuário, em três arquivos com ciclo de vida próprio: endpoints e credenciais por fonte, camadas e injeção de dependências, empacotamento e CI. A arquitetura é arquivo próprio e não seção do `CONTRIBUTING.md` porque contribuição é procedimento lido uma vez e arquitetura é referência consultada muitas. **Este commit deixa uma âncora pendurada de propósito** — `../README.md#supported-integrations` só passa a resolver em A07, quando o README novo entra. É a única quebra intermediária da sequência, e a alternativa seria um commit único de 1.300 linhas |
| A04 | 2026-08-29 | `docs: add a contributing guide` | `CONTRIBUTING.md` | concluída | 107 linhas: JDK 17, comandos, política de teste, convenções, regras visuais e fluxo de PR. **Sem a linha `git config user.name "codex"`** que estava no README público — é regra de agente e já vive em `AGENTS.md`. Inclui as duas armadilhas que já custaram suíte vermelha (`delay` em `runTest` sobre `Dispatchers.Default`, e animação infinita travando `waitForIdle`), porque nenhuma das duas é derivável da leitura do código |
| A05 | 2026-08-29 | `docs: add a security policy` | `SECURITY.md` | concluída | 87 linhas com o modelo de dados, que aqui é matéria de segurança real: a tabela do que é lido separa **leitura** de **escrita** — a única escrita é a renovação do token OAuth da Anthropic —, e a dos segredos registra por que eles não vivem no `PreferencesSettings`, que é gravado em claro. Registra também que os instaladores não são assinados e que a integridade vem do `digest` da API do GitHub. **Descoberta que mudou o texto:** `gh api repos/.../private-vulnerability-reporting` devolve `{"enabled":false}`, então o canal que o documento indica ainda não existe — habilitá-lo entrou em A08 |
| A06 | 2026-08-29 | `docs: add a changelog` | `CHANGELOG.md` | concluída | **Sem backfill das 67 tags.** As notas de cada uma já existem, geradas pelo workflow a partir de assuntos de commit; transcrevê-las daria ~2.000 linhas duplicando a página de Releases verbatim, e faria o repositório parecer ter um changelog mantido que ele não teria. O arquivo aponta para lá como histórico completo e recebe curadoria a partir da próxima versão. Nenhum passo de CI escreve nele: seria commit de bot sobre um commit já tagueado, brigando com o fluxo disparado por tag |
| A07 | 2026-08-29 | `docs: rewrite the readme in english with a portuguese mirror` | `README.md` reescrito + `README.pt-BR.md` | concluída | **582 → 269 linhas** (-54%), com a instalação saindo da linha 471 para a 124 — depois de recursos, integrações e telas, em vez de enterrada em `Build e distribuicao`. **Último commit da sequência de propósito:** o README é o hub que linka os seis documentos anteriores, e qualquer outra ordem deixaria links quebrados em mais de um commit intermediário. O espelho PT entra junto porque o seletor de idioma de cada um aponta para o outro. **Defeito achado pela verificação, não pela leitura:** a primeira redação omitiu a seção `## Supported integrations`, e a âncora que `docs/integrations.md` publica ficou sem alvo — encontrado pelo script que resolve cada âncora contra os headings reais do arquivo de destino. **Nenhuma imagem foi apagada e nenhuma ficou órfã:** as quatro sem referência são geradas por `ScreenshotGenerator.kt:101,108-110`, então `rm` seria no-op e sujaria o worktree na passada seguinte; as 14 de `img/` passaram a ser referenciadas, verificado por laço sobre `ls img/`. **Nenhuma mudança de código em toda a passada:** `grep -rn README --include=*.kt --include=*.kts` devolve 11 ocorrências, todas em comentário e nenhuma citando seção por nome, então renomear e traduzir todos os headings não deixou referência falsa. `gradlew.bat desktopTest --rerun`: **154 classes / 1632 testes / 0 falhas / 0 erros / 0 pulados** em 2m04s — `allTests` devolveu `UP-TO-DATE` sem executar nada, comportamento que o `CLAUDE.md` já registra |
