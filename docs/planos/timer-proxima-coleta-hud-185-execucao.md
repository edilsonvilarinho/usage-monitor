...
| A03 | `HudBar` compõe a contagem na primeira linha | `gradlew.bat desktopTest --tests "com.usagemonitor.ui.HudBarCountdownTest" --tests "com.usagemonitor.ui.HudBarHeightTest"` | `BUILD SUCCESSFUL`, **6 testes novos, 0 falhas**, e os 5 de altura intactos. **A primeira passada não terminou: o laço do relógio trava o `waitForIdle`.** Sob o clock dos testes de componente o `delay` avança sozinho e o `nowProvider` fixo nunca deixa a contagem chegar a zero — o laço gira para sempre. É o mesmo motivo pelo qual o `FooterBar` já tinha `countdownUpdatesEnabled`, e o `HudBar` ganhou o mesmo interruptor; quem exercita o decremento injeta `waitNextTick` com um `Channel`, quem só quer o número na tela desliga o laço. **O `weight` da linha de carregamento mora no indicador**, não num `Spacer` próprio: a `HudPanelRow` espaça os filhos, e um terceiro filho traria um vão que `hudFallbackRowWidth` não conta — a janela nasceria estreita |
| A04 | Fiação em `Main.kt` e o rótulo com dono único | `gradlew.bat desktopTest --tests "com.usagemonitor.ui.HudBar*" --tests "com.usagemonitor.ui.FooterBarTest" --tests "com.usagemonitor.HudWindowGeometryTest"` | `BUILD SUCCESSFUL`. `HudBarHeightTest` foi de 5 para **8 testes** — parada, aberta e na linha de carregamento, todas com a contagem — e os **9** do rodapé continuam intactos. `main()` ganhou **duas linhas e nenhuma composable nova**, pela razão registrada no CLAUDE.md. A frase "Próxima atualização automática" saiu de dentro do `FooterBar` e virou `nextRefreshLabel(language)`: ela é a tooltip do rodapé e a descrição semântica do ícone do HUD, e escrita nos dois lugares uma tradução corrigida num deles deixaria o outro para trás. `showsCountdown = true` entra nas **duas** medidas — a âncora descreve o painel completo, e medi-la sem a coluna faria a janela saltar ao expandir |
| A05 | Contrato do `AppHudBar` no design system | inspeção do diff | `AppHudBar.prompt.md`, `.jsx`, `.d.ts` e o kit `ui_kits/desktop-app/Hud.jsx`. **Uma divergência deliberada ficou escrita em vez de escondida:** o componente de referência recebe o valor pronto e o do Compose tica por dentro — lá o host é o composable de janela que monta o grafo inteiro, e um estado por segundo nele recomporia tudo. O primeiro rascunho do `.d.ts` afirmava que o host é quem tica, o que o Kotlin não faz; foi corrigido antes do commit. O validador de tags que rodei é fraco (reprova o arquivo original do mesmo jeito), então a conferência foi por leitura do diff |
| A06 | Protótipo, § Barra HUD | conferência de tags (`div` 843/843, `span` 1012/1012, `p` 104/104, `svg` 18/18, `path` 30 auto-fechados) | a coluna nova entrou nos três mockups com painel, e os mockups **2 e 4 foram alinhados ao desenho vigente**: eles ainda desenhavam uma linha por *cota* com coluna de reset, que o Kotlin abandonou quando a linha virou por conta — pela regra de precedência, o design system já descrevia a versão certa e quem se corrige é o protótipo. Duas notas mentiam junto com eles: a do reset por cota e a do rótulo `{perfil ou fonte} · {cota}`. Nota nova sobre a contagem sair uma vez só, sumir no ponto e ficar na linha de carregamento |
| A07 | `CLAUDE.md`, `CHANGELOG`, catálogo de ajuda e a demo | `gradlew.bat generateHelpMedia` + inspeção do quadro gerado | `window-modes.gif` regerado: **177 KB** (era 168), 1000×420, 26 quadros. **As outras onze foram restauradas com `git checkout --`**: o gravador dorme em tempo real e cada passada produz bytes diferentes, então commitá-las seria ~1 MB de churn binário sem informação nova. O quadro extraído confirma o desenho — `↻ 02:05` na primeira linha, ausente nas outras duas. Na demo o laço vai **desligado** e o relógio é fixo, senão cada passada gravaria um número diferente. O texto do tópico 10 do catálogo passou a citar a contagem nos dois idiomas |
| A08 | Verificação e o teto que a medição obrigou a subir | `gradlew.bat allTests` + medição das contas reais + `gradlew.bat run` | `BUILD SUCCESSFUL`, **1858 testes, 0 falhas**, 176 classes. **O risco 1 se confirmou e foi medido, não estimado:** com a coluna, `Anthropic — Padrão` vai de 356,9 para 420,9dp e `OpenCode Go` de 361,6 para 425,6 — os dois cabiam e passariam a truncar. O teto subiu para **484**, que é exatamente o teto anterior mais a largura da coluna: manter 420 faria a coluna nova ser paga pelo nome da conta, que é a mesma razão do salto 320 → 420. **O teste `os valores de contrato continuam onde estavam` reprovou a mudança**, que é o trabalho dele; o valor só se moveu com a razão escrita junto. **O que não foi verificado:** o comportamento interativo da janela (arrasto, encaixe na borda, crescer/encolher no hover) — `gradlew.bat run` saiu como `second-instance-exit` porque há uma instância v38.1.0 do app aberta na máquina, e encerrá-la seria mexer na sessão de quem está usando. O que dava para ver sem isso foi visto: o quadro gerado da demo mostra a barra composta com `↻ 02:05` na primeira linha e ausente nas outras |
