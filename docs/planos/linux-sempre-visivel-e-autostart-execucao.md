...
| R6 | A fiação da A14 dentro de `ensureAutoStartCommandCurrent()` **não é exercitada pela suíte**: a função lê `~/.config/autostart` e `currentPlatform()`, e no Windows o ramo do Linux nunca roda. O que os testes cobrem é a decisão pura (`linuxAutoStartNeedsRepair`). A prova de que a migração dispara de fato é a A13, no logon real da Bazzite | aberto |
