...
| A03 | Fiação em `DashboardScreen` e `Main.kt` | `gradlew.bat desktopTest --tests "com.usagemonitor.ui.ComponentTest" --tests "com.usagemonitor.ui.FooterBarTest"` | `BUILD SUCCESSFUL`, 0 falhas: `ComponentTest` fechou em **91 testes** (90 antes) e os 13 do rodapé seguem verdes. **Nenhuma composable nova em `main()`** — dois parâmetros a mais na chamada do `DashboardScreen`, que já repassava outros cinco. O `when` que deriva a moldura corrente lê os **dois booleanos** na ordem `hudMode` → `cardsOnlyMode` → padrão, e a troca chama os setters que já existiam: é neles que a exclusão mútua mora, e duplicá-la aqui abriria um segundo dono da regra. O teste novo da `DashboardScreen` cobre o **repasse**, que é o que os testes do `FooterBar` não alcançam: sem ele o menu existiria na primitiva e nunca chegaria à tela |
| A04 | Contrato do `AppMenu` no design system | conferência de tags do JSX contra a versão anterior (`div` 4/4 e `span` 3/2 no kit, idênticos ao baseline; no componente novo, `div` 3/2 pela sobreposição autofechada) | `components/core/AppMenu.{prompt.md,jsx,d.ts}`, a entrada no índice do `readme.md` e o rodapé do kit `ui_kits/desktop-app/Dashboard.jsx`. O contrato escreve as quatro decisões que o Kotlin tomou — menu × segmentado, por que não é o menu da plataforma, marca além do realce com coluna reservada, e abrir para cima —, mais a regra de sempre sobre animação. **Uma divergência deliberada ficou escrita:** o componente de referência posiciona por CSS (`placement`) e o do Compose por `PopupPositionProvider`, porque lá o popup é camada dentro da janela e precisa ser preso aos limites dela nos dois eixos |
| A05 | Protótipo | conferência de tags (`div` 852/852, `span` 1021/1021, `p` 110/110, `button` 187/187) | o rodapé do mockup do dashboard ganhou o ícone, e o **estado de menu aberto** virou seção `h3` própria dentro da tela que já existe — não é tela nova, então não entra no `nav.index`, mesmo tratamento do § Modo somente cards e do § Barra HUD. Regras `.menu`/`.mitem` novas no CSS do próprio protótipo, com a marca de seleção ocupando lugar em toda linha. Quatro notas: menu × segmentado, abrir para cima, marca além do realce, e o menu existir só no modo padrão |
| A06 | `CLAUDE.md`, `CHANGELOG`, ajuda e demo | `gradlew.bat desktopTest --tests "com.usagemonitor.*Help*"` + `gradlew.bat generateHelpMedia` + inspeção do quadro composto | `BUILD SUCCESSFUL` nos testes de ajuda; `window-modes.gif` regerado (**159 KB**, 1000×420, 26 quadros) e as outras onze restauradas com `git checkout --`. O quadro extraído confirma o rodapé com **quatro** ícones: modo de janela, atualizar, configurações e ajuda. **O menu aberto ficou fora da demo, deliberadamente**: ele é popup, e o gravador compõe a cena offscreen, onde a camada de popup não entra no quadro — gravar assim mostraria um clique sem resposta. **As capturas do README não foram regeradas**: elas passam pelo `ScreenshotGenerator`, que monta o rodapé sem o callback, e regerar PNGs por um ícone seria churn binário sem informação nova — fica registrado como lacuna conhecida. O passo novo da ajuda cita o rótulo real do controle ("Modo de janela") e diz que o menu só existe no modo padrão, nos dois idiomas |
| A07 | Verificação | `gradlew.bat allTests` + `gradlew.bat run` com clique sintético e captura de tela | `BUILD SUCCESSFUL`, **1863 testes, 0 falhas**, 176 classes (1858 antes). **No app real** (instância anterior encerrada com o usuário ciente e a janela devolvida ao modo normal ao fim): o ícone aparece como primeiro do grupo de ações; o clique abre o menu **para cima**, sobre os cards, com `Padrão` marcado e realçado; escolher `Barra HUD` encolhe a janela à pílula na hora. O **risco 1 não se materializou**: o popup não foi recortado. **Um falso negativo de harness, e vale registrar:** os três primeiros cliques sintéticos não abriam nada, nem no ícone novo nem na engrenagem que já existia. A causa era o PowerShell rodar sem consciência de DPI: `SetCursorPos` recebia coordenada lógica e o sistema a escalava, e o clique caía em outro ponto. Com `SetProcessDPIAware` a tela virtual passou de 1920×1880 para 2880×2820 e o mesmo clique acertou. Sem essa checagem — clicar num controle **que já funcionava** — o menu teria sido dado como quebrado |
