...
- **O artefato de 122 MB não é podado depois de aplicado** — segue em `~/.usage-monitor/updates/`.
  A contra-auditoria já suspeitava; agora está medido. Não corrigido aqui: virou a issue #87, fechada
  pela **A21**.
...
| A21 | 2026-08-24 | `fix(update): discard the update artifact once it has been applied` | Fecha a issue #87 | concluída | O `Setup.exe` de ~120 MB ficava em `~/.usage-monitor/updates/` depois de aplicado: a única poda existente roda em `prepare()`, ou seja, só quando uma versão **nova** é anunciada — sem versão nova, nunca há poda. A decisão saiu como função pura no domain (`shouldDiscardUpdateArtifacts`), e as duas condições dela importam: **falha mantém o artefato** (o download é retomável e um arquivo íntegro no disco não toca a rede) e **versão diferente da que está rodando é prova de que a troca não aconteceu**. A poda vive num `LaunchedEffect` dentro de `rememberAutoUpdateController`, não no `main()` — aquele composable não pode receber estado novo — e não no corpo do `remember`, porque apagar arquivo é escrita. Idempotente, então não há marcador a guardar; e o recibo **não** é apagado, porque ainda alimenta a linha das Configurações. `prune` virou a top-level `pruneUpdateArtifacts(directory, keepAssetName)` com `null` = apagar tudo, para quem poda na abertura não precisar de um `HttpClient`. **Verificado em execução, nas duas direções**: com recibo `status=success version=37.0.0` (a do build) o `gradlew run` deixou `updates/` **vazio**; com `status=failed` o arquivo **permaneceu**. `gradlew.bat desktopTest --rerun`: **116 classes / 1240 testes / 0 falhas** (base do merge do PR #86 era 115/1234; mais a classe nova da decisão e os dois casos de poda) |
