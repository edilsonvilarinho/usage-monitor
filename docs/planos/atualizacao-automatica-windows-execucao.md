...
| — | 2026-08-22 | `docs(plan): record that the desktop wiring is not runtime-verified` | **Verificação pendente da A13** | aberta | `gradlew.bat run` terminou em 2 s sem compor nada: o `Usage Monitor.exe` instalado estava em execução e o `SingleInstanceGuard` derrubou o processo de desenvolvimento antes do `main()`. Ou seja, **a fiação do `AutoUpdateController` não foi exercitada em execução** — só compilada e coberta por teste unitário, e `main()` não tem teste de componente. Fechar o app instalado e repetir o `gradlew.bat run` é o que falta; fica junto da A20 |
