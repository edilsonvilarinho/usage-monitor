...
| **R14** | A Section `Uninstall` do `.nsi` roda `taskkill /F /IM java.exe`, que mata **toda JVM da máquina** — daemons do Gradle e IDEs de quem instala. Roda também dentro dos cenários, via o desinstalador | leitura do `.nsi`; comportamento pré-existente, não introduzido aqui | fora deste escopo — merece issue própria | aberto |
