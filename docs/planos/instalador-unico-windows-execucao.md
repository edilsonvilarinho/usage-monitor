...
| **A06b** | Não prevista: `prepareInstallerFiles` deixa de acumular jars de builds anteriores | `build.gradle.kts` | `fix(packaging): clear the installer payload before staging it` |
...
| A05 | 2026-08-24 | `feat(installer): remove a previous MSI install before installing` | Remoção do MSI por baixo no `.onInit` | concluída | Quinto `!ifndef` (`MSI_UPGRADE_CODE`) e `Function RemoveForeignInstall`, chamada no ramo que não é `/UPDATE` e antes de qualquer escrita. Compilação com os cinco `/D` sobrescritos: `makensis` exit 0, só os **dois warnings pré-existentes** de `MUI_TEXT_FINISH_RUN` — fecha **R8**. **Prova em execução, ponta a ponta:** MSI real do v37.0.0 instalado, app **aberto e com 2 processos**, e o setup rodado com `/S`: exit **0 em 3,2 s**, app fechado (0 processos), MSI removido dos três lugares (ARP, pasta e vínculo de UpgradeCode), payload novo gravado e `team.json` com o mesmo SHA-256. Antes disso, o caminho negativo: com um UpgradeCode que não existe na máquina, o setup instalou normalmente e não tocou em nada. Isolamento conferido nas duas passadas — chaves e atalhos saíram sob `UM Syntax Check`/`UM Msi Takeover`, e foram removidos ao fim |
...
| A09 | 2026-08-24 | `fix(installer): stop killing every JVM on the machine` | Fecha o **R14** (issue #88) | concluída | A linha `taskkill /F /IM java.exe` saiu da Section `Uninstall`, e o `taskkill` que resta passou a `/F /T /IM "${PRODUCT_NAME}.exe"` — o mesmo tratamento que a A07 já dera ao de `RemoveForeignInstall`, mais `/T`, porque o app instalado são **dois** `Usage Monitor.exe` em cadeia (medido: 44916 → 47908). O mesmo bloco saiu de `build-with-icon.ps1`, que derrubava o daemon do Gradle de quem builda. **S9 novo no roteiro**: uma JVM sentinela (`java.exe Sentinel.java`, single-file source launcher) fica viva durante a rodada, e S5/S6/S8 acionam o desinstalador pelo `ExecWait '$0 _?='` do `.onInit`. Sem `java.exe` resolvível o cenário é **pulado com aviso**, no mesmo tratamento que o S7 dá à ausência do WiX — o job `installer-scenarios` não tem `setup-java`. **Falsificado nas duas direções:** com a linha reintroduzida a rodada devolveu `Verificacoes: 40 Falhas: 1`, exit 1, e a máquina ficou com **zero** `java.exe`; restaurada, `Verificacoes: 40 Falhas: 0`, exit 0, sentinela vivo. S7 pulado nas três rodadas (sem WiX local) |
...
|---|---|---|---|---|
| **R1** | WiX não está instalado nesta máquina (`$env:WIX` vazio, nenhum `candle.exe`) — S7 não roda localmente | **resolvido na A07**: o roteiro aceita `-WixBinPath` e pula o S7 com aviso quando não acha `candle.exe`; o S8 nunca precisou de WiX. Localmente o S7 foi provado com os binários portáteis do WiX 3.11.2 no scratchpad, e o CI ganhou `choco install wixtoolset -y` | A07 | fechado |
...
| **R8** | Sintaxe do `System::Call` para `MsiEnumRelatedProductsW` (buffer de saída em `w .r1`) só é validada ao compilar e executar | **fechado na A05**: `makensis` exit 0 sem warning novo, e a chamada foi exercitada em execução nas duas direções — com produto presente (removeu) e ausente (seguiu em frente) | A05 | fechado |
...
| **R13** | `prepareInstallerFiles` é um `Copy` do Gradle **sem limpar o destino**, e `Copy` é aditivo: jar de build anterior cujo hash mudou fica em `build/installer/files` e **entra no instalador**. Medido em 2026-08-24: 2 jars órfãos, 4.464.723 bytes a mais no payload e o `Setup.exe` em 126.532.759 bytes contra os ~122,3 MB registrados na A15/A16 da #75 | **medido**; defeito pré-existente, invisível no CI porque lá o workspace nasce limpo | A06b | **fechado** |
| **R14** | A Section `Uninstall` do `.nsi` roda `taskkill /F /IM java.exe`, que mata **toda JVM da máquina** — daemons do Gradle e IDEs de quem instala. Roda também dentro dos cenários, via o desinstalador | **medido em 2026-08-24 na #88**, nas duas direções: com a linha presente, uma rodada da suíte deixou **zero** `java.exe` vivos na máquina — o daemon do Gradle (pid 39440) e o compilador Kotlin (pid 44556) morreram; sem ela, os dois sobrevivem. E o `taskkill` nunca encerrou nada do produto: a árvore instalada é app-image do jpackage, `runtime/bin` só tem `.dll`, e o app roda como dois `Usage Monitor.exe` em cadeia (44916 → 47908) | issue #88 — a linha saiu, e `/T` sobre `${PRODUCT_NAME}.exe` virou a rede de segurança restrita ao produto | **fechado** |
| **R15** | `ExecWait` sobre o desinstalador do NSIS **não espera** sem `_?=`: ele se copia para `%TEMP%` e o processo original retorna na hora | **medido**: S6 reprovou com `v1` esperado e `<ausente>` obtido, mais atalho e chave `Run` apagados por baixo da instalação nova | A07 (`_?=` no `ExecWait`) | fechado |
