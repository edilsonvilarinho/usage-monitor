...
- A flag foi ligada na **A14** (2026-08-29), depois do aceite em máquina real — issue #121 relata a
  investigação inteira, incluindo os dois defeitos que o aceite expôs e que não estavam neste plano.
...
- **O aceite real cobriu os passos 1–3 do roteiro** (instalação gerenciada, download com SHA-256 real,
  swap com recibo `success`), numa Bazzite/rpm-ostree. Os passos 4–6 (botão de reinício manual, rollback
  por falha de lançamento induzida, interruptor desligado sem downloads) não foram reexercitados nesta
  rodada — a evidência ali é o harness POSIX (45/45) e as corridas reais anteriores.
...
| A07 | 2026-08-25 | `feat(update): ship the linux updater as a versioned resource` | `linux-updater.sh` + materialização em `0700` | concluída | Recurso versionado, materializado em `0700` e executado por `/bin/sh` com os oito posicionais **separados** no `ProcessBuilder` — nenhum caminho interpolado dentro do arquivo. Script sobrado é **apagado** antes de escrever o próximo, não sobrescrito: o `rm -f "$0"` da última linha pode não ter rodado, e sobrescrever manteria permissão e inode antigos. **Dois defeitos achados por executar, não por ler.** (1) O ramo `launch-failed` era inalcançável: `if ! ( cmd & )` devolve 0 sempre, e `kill -0` sobre o filho também não serve porque sem `wait` um processo morto vira zumbi e continua respondendo — a detecção virou a checagem do executável promovido, que é o que dá para afirmar sem bloquear. (2) `promoted_launcher=$target_dir/Usage Monitor/…` **sem aspas** é atribuição seguida de comando: o script morria com `Monitor/bin/Usage: No such file or directory` **depois** de já ter escrito `current`, e nenhum rollback rodava. `sh -n`: sintaxe OK. Três execuções reais em Git Bash — sem marcador (aborta, `current` intacto, staging preservado), caminho feliz (swap, ACK, recibo `success`, poda deixando a atual e **uma** anterior, staging limpo, script autoapagado) e falha de lançamento (`current` restaurado para 38.0.0, recibo `failed reason=launch-failed`). `gradlew.bat desktopTest --tests "…LinuxUpdaterScriptTest"`: 9 testes, 0 falhas |
| A08 | 2026-08-25 | `feat(update): apply linux updates from the managed xdg tree` | `LinuxAppUpdateInstaller` + piso de versão-alvo + dispositivo nulo por SO | concluída | `support()` na ordem plataforma → **arquitetura** → origem: numa ARM64, responder *origem sem suporte* prometeria que reinstalar resolve. Seleção estrita `LINUX`+`X64`+`LINUX_TARBALL`+digest — o `.deb` e o `.rpm` da mesma release são artefatos Linux x64 legítimos e recusados de propósito, porque aplicá-los exigiria gerenciador de pacotes e `sudo`. `MIN_LINUX_UPDATABLE_TARGET_VERSION` sai no sentinela `999.0.0`: nenhuma release é alvo aceito enquanto ele estiver de pé, e o motivo é o mesmo do gate do Windows com outro mecanismo — um binário anterior a este código ignora o argumento privado, sobe, nunca confirma, e o script desfaz uma atualização boa. Staging sobrado é **apagado** antes de extrair: extração interrompida não se distingue de completa. `launchDetachedProcess` resolve o dispositivo nulo por SO — `NUL` fixo é caminho relativo inexistente no Linux, e `Redirect.from` **abre o arquivo**: seria o mesmo desenho de falha do `Redirect.DISCARD` da A20 — e ganhou um terceiro parâmetro para a saída ir para o log do updater, em modo append, sem o script conhecer o caminho. A classe é `internal` porque expõe `LinuxInstallLayout` e `TarballExtractor` na assinatura; público ali não compila. `gradlew.bat desktopTest --tests "com.usagemonitor.update.*"`: 0 falhas |
| A09 | 2026-08-25 | `feat(update): pick the installer by platform, linux off` | Seleção por plataforma + `LINUX_AUTO_UPDATE_SHIPPED = false` | concluída | `rememberAutoUpdateController` passa a escolher **um** instalador por plataforma. Antes ele construía o do Windows em qualquer lugar e dependia de ele responder `UNSUPPORTED_PLATFORM`; com dois instaladores isso viraria construir o errado e confiar na recusa. **Consequência que precisou de correção:** com `installer` nulo, `support` caía em `UNAVAILABLE` para todo mundo, e no macOS isso é **falso** — ali não falta build, falta assinatura, e a tela prometeria que uma versão futura resolveria. `support` passou a derivar o motivo da plataforma quando não há instalador. `AutoUpdateWiringTest` ganhou o portão do Linux nos dois sentidos, mais um caso afirmando que os dois gates são independentes: são mecanismos diferentes exercitados em máquinas diferentes. `gradlew.bat desktopTest --rerun`: **130 classes / 1408 testes / 0 falhas** (linha de base 1310) |
| A10 | 2026-08-25 | `feat(autostart): point the linux entry at the stable launcher` | Autostart no launcher estável + migração | concluída | A entrada Linux passa a nomear `~/.local/bin/usage-monitor`, que lê `current` e sobrevive à troca de versão. Uma entrada apontando para dentro de `versions/<versão>` funciona **até a primeira atualização**: a árvore que ela nomeia é podada dois ciclos depois, e a falha é silenciosa porque nada no app relê a entrada depois de escrevê-la. O teste de escolha é a **presença do arquivo executável**, não o resolvedor de origem: numa instalação `.deb` o launcher simplesmente não está lá e o fallback é o que sempre foi. `ensureAutoStartCommandCurrent` ganhou o segundo critério; **entrada ausente continua não migrando**, pelo mesmo motivo do argumento `--autostart` — ligaria a inicialização de quem a desligou —, e entrada que já nomeia o launcher, ou que aponta para `/opt`, é deixada em paz. As duas decisões são funções puras com os lados injetados, porque a suíte roda no Windows e ali não existe nem `~/.local/bin` nem bit de execução. `gradlew.bat desktopTest --tests "…AutoStartManagerTest"`: 0 falhas |
| A11 | 2026-08-25 | `build(release): validate the linux tarball and ship its installer` | Validação do tar + `install-usage-monitor_<versão>_linux_x64.sh` | concluída | O tarball deixou de ser anexo de conveniência: é o pacote que a atualização extrai e promove, e um `.tar.gz` com duas raízes, com link, ou sem `0755` no launcher só falharia **depois** do swap — quando o app já não está de pé para relatar. O workflow ganhou as mesmas guardas do `TarballExtractor`, do lado de lá. O instalador sai de um template com `@VERSION@`/`@TARBALL@`/`@SHA256@`, e o passo **reprova placeholder sobrado**: template e workflow fora de sincronia produziriam um instalador baixando a URL literal `@TARBALL@`. O SHA-256 daqui serve **só** ao instalador inicial, que não fala com a API — quem se atualiza sozinho usa o `digest` da release. `artifactKindOf` devolve `null` para `.sh` e o asset é descartado pelo cliente: nenhuma mudança de código. Verificado localmente: `sh -n` no template substituído e uma **instalação real** num `$HOME` falso — marcador, `versions/39.0.0/Usage Monitor`, `current`, launcher e `.desktop` criados, SHA conferido, nada fora do home. YAML validado com `yaml.safe_load`. O layout tem dois donos (template e Kotlin) e `LinuxInstallLayoutTest` passou a amarrar os **valores** às constantes — divergência ali não dá erro, dá um app que não encontra a própria instalação |
| A12 | 2026-08-25 | `ci: run the linux updater scenarios on three shells` | Harness POSIX + job `linux-updater-scenarios` | concluída | **11 cenários, 45 verificações, 0 falhas**, medidas localmente em Git Bash: swap com ACK, processo anterior demorando, timeout de PID sem alterar `current`, falha de lançamento com rollback, health timeout com rollback e relançamento, diretórios com espaço, staging ausente, versão que escaparia da raiz, marcador ausente, poda mantendo a atual e **uma** anterior, e promoção da versão em execução recusada. **Defeito achado na primeira execução do roteiro:** ele invocava o updater direto do repositório, e o `rm -f "$0"` do caminho de sucesso **apagou o recurso versionado** — restaurado por `git checkout`; o roteiro passou a copiar o script para o diretório do cenário, que é também o que o app faz. Os tetos de PID e de ACK viraram ajustáveis por ambiente, com os defaults de 60 s no arquivo: um teste que leva 60 s para provar que um timeout dispara é um teste que ninguém roda. O job usa **três containers** (`ubuntu:24.04`, `archlinux`, `fedora`) porque a promessa do script é `/bin/sh`, e ali `/bin/sh` é `dash` no primeiro e `bash` nos outros dois — um bashismo que passe no Fedora quebra o update de quem usa Debian. O `git` é instalado **antes** do checkout: sem ele a action cai no download por API, o `git diff` do filtro falha e o job rodaria os cenários sempre, verde e sem o filtro que deveria ter. YAML validado com `yaml.safe_load`. **Segundo defeito, e o mais grave da atividade:** o `git checkout` que restaurou o updater o devolveu com **CRLF** — `core.autocrlf=true` na máquina de desenvolvimento. Em Linux o kernel lê a linha do shebang inteira, então `#!/bin/sh<CR>` procura um interpretador chamado `/bin/sh^M` e o updater falharia para **todo** usuário, sem que nenhum `sh -n` local percebesse. `.gitattributes` novo prende `*.sh` e `*.sh.template` em `eol=lf`, os três arquivos foram normalizados, e `LinuxUpdaterScriptTest` ganhou o caso que reprova CR no recurso empacotado |
| A13 | 2026-08-25 | `docs: describe the linux user-space auto update` | README + protótipo `§15 #checklist` | concluída | A seção de atualização automática deixou de dizer *Windows* no título e ganhou o bloco do Linux: o layout da árvore, o fluxo de promoção com espera de confirmação, o rollback em 60 s, os ~125 MB por versão e o log. A tabela de indisponibilidade passou de duas linhas para **quatro**, com Linux fora da instalação user-space e ARM64 separados — a segunda não é "plataforma sem suporte", é pacote que não existe. Seção nova **Instalação no Linux** com os três formatos e qual deles se atualiza sozinho. Protótipo `§15` com cinco linhas: a flag desligada por build e o porquê, os ~600 MB com duas versões retidas, o tarball sem assinatura, a migração manual de `.deb`/`.rpm`, e a regra fixa de que sem marcador o script aborta sem tocar em nada |
| A14 | 2026-08-29 | `feat(update): ship linux auto update after real bazzite acceptance` | Liga `LINUX_AUTO_UPDATE_SHIPPED` + fixa o piso em `38.0.1` | concluída | **A máquina real apareceu**: issue #118 reportada numa Bazzite (rpm-ostree). A migração do `.rpm` para o `.sh` user-space expôs dois defeitos que bloqueavam a A14 havia meses e nenhum dos dois estava no plano original — achados por medição ao vivo, não por leitura. **(1)** `LinuxInstallOriginResolver` comparava `jpackage.app-path`/`ProcessHandle` (resolvido pelo kernel, `/var/home/...`) contra `user.home` sem canonicalizar (`/home/...`, symlink ostree não resolvido): toda instalação `.sh` legítima caía em `UNMANAGED` — commit `f7bbfc2`. **(2)** o processo relançado pelo passo 6 do script herdava o `LD_LIBRARY_PATH` que a JVM que estava saindo tinha setado, apontando para o `lib/app` da versão **anterior**; o launcher nativo do jpackage usa essa variável para se autolocalizar e, com ela errada, pulava a própria etapa de autoconfiguração e saía sem chegar em `main()` — o processo relançado **nunca aparecia em `ps`** durante toda a janela de ACK. `unset LD_LIBRARY_PATH` antes dos dois pontos de lançamento do script — commit `d937355`. Foram documentados na issue #121 antes de fechar, com os dois cenários de falha capturados via `strace -v -s 4000`. **Endurecimentos no mesmo lote:** o token do ACK saiu de `--update-ack=X` (vazava como opção da própria JVM no launcher nativo — `Unrecognized option`, medido ao vivo) para a variável `USAGE_MONITOR_UPDATE_ACK`; a saída dos dois pontos de lançamento passou de `/dev/null` para `linux-update.log`, anexada — foi essa mudança que tornou o defeito (2) diagnosticável. **O aceite real**, feito nesta mesma Bazzite depois da v38.0.1 publicada com os dois fixes: um binário local (`current`=`37.9.9`, `LINUX_AUTO_UPDATE_SHIPPED=true`, piso `0.0.0`, JDK 17 — a imagem `createDistributable` sai quebrada com "pure virtual method called" se o daemon do Gradle sobe em JDK 21) trocado para dentro da árvore gerenciada real via `~/.local/bin/usage-monitor`, checou o feed real do GitHub, baixou o tarball verdadeiro da v38.0.1, promoveu e relançou — `linux-update.log` registrou `OK promoted 38.0.1` e `update-receipt.properties` saiu `version=38.0.1 previousVersion=37.9.9 status=success`, o primeiro ciclo completo fechado sem rollback desde que o mecanismo existe. **Cobertura parcial do roteiro de seis passos**: os passos 1–3 (instalação gerenciada, download com SHA-256 real, swap com recibo de sucesso) têm evidência direta desta corrida; os passos 4–6 (botão "Reiniciar e atualizar agora", indução de falha de lançamento com rollback, interruptor desligado sem downloads) não foram reexercitados ao vivo nesta rodada — ficam cobertos pelos 45/45 do harness POSIX e pelas execuções reais anteriores (A07/A12), não por uma nova observação numa máquina real. `LinuxAppUpdateInstallerTest` tinha dois testes acoplados ao **valor** do sentinela, não ao mecanismo — `minTargetVersion` passou a ser injetado explicitamente como `999.0.0` nos dois, em vez de vir do default de produção. `gradlew allTests`: **1472 testes / 4 falhas**, as mesmas `WindowsInstallOriginTest` pré-existentes, Windows-only, rodando neste ambiente Linux. Harness POSIX: 45 verificações, 0 falhas |
